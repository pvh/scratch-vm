 ...
 */
// This function is actually kind of pointless: we already HAVE the data. We just want to get it into objects.
const parseScratchObject = function (object, runtime, extensions, zip, assets) {
    ...
    const sprite = new Sprite(
        blocksObj,
        runtime,
        object[Symbol.for('_am_objectId')]
    );
    ...
    // TODO: not sure what these are or how important they are r/n
    ...
    if (Object.prototype.hasOwnProperty.call(object, 'broadcasts')) {
        for (const broadcastId in object.broadcasts) {
            const broadcastObj = object.broadcasts[broadcastId];
...
const deserializeMonitor = function (
    ...
    const existingMonitorBlock = runtime.monitorBlocks.getBlocks(monitorData.id);
 ...
 */
const deserialize = function (unusedJson, runtime, zip, isSingleSprite) {
    const json = runtime.handle.docSync();
