...
export default class VirtualMachine extends EventEmitter {
     ...
     */
    // TODO: i planned here to have the handle as input but the project-fetcher
    // bypasses us and i'm staying out of gui for now
    loadProject (input) {
        if (
            typeof input === 'object' &&
     ...
     */
    // TODO: This should take a docURL and tell the repo to find it()
    // TODO: This isn't the only entry point!  There's a project-fetcher-hoc that bypasses this!
     ...
     * @returns {Promise} Promise that resolves after the project has loaded
     */
    // TODO: we're gonna ignore all the zip stuff and chop out sb2/3 support for this prototype
    // we should already have an automerge doc by now, so just keep rolling with that:
    // the strategy is to push down bits of the doc into the sprites & blocks code
    // (one big problem is the runtime is mucking in the guts of other classes here; will have to put an end to that)
    // at this point the document has { meta, extensions, monitors, and targets }
    // (we can ignore the first three mostly for now)
    deserializeProject (json, zip) {
        ...
        const deserializePromise = async function () {
            ...
            // the Sprites and Blocks, in particular

            const am = require('./serialization/automerge');
            await runtime.handle.whenReady();
            return am.deserialize(json, runtime, zip, false);
        };
        return deserializePromise().then(({targets, extensions}) => {
            ...
            // by now i think we should be mostly hands-off
    ...
    installTargets (targets, extensions, wholeProject) {
        ...
        return Promise.all(extensionPromises).then(() => {
            ...
            // TODO: it would be best to avoid mutating the internal runtime state from outside of runtime
            // there are a few patterns like this; we should probably make this cleaner
            /*
            // PVH disabled all this for now
     ...
     * @return {!Promise} Promise that resolves after targets are installed.
     */
    // TODO: this isn't a priority but for completeness at some point we should do this one too
    addSprite (input) {
     ...
     */
    // TODO: eventually this API could refer to costumes in automerge documents but let's just live without that for now
